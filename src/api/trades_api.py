...
def _fetch_live_balance() -> float:
    """Fetch USDC cash balance via Polymarket CLOB API (balance-allowance).

    Uses signature_type=1 (POLY_PROXY) which matches the proxy wallet setup.
    Falls back to on-chain USDC.e balance via Polygon RPC if CLOB fails.
    """
    ...
    # Fallback: on-chain USDC.e via RPC
    import httpx
